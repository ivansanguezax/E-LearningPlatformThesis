...
const CourseContentMedia = ({
  ...
  const handleQuestion = () => {
    if (question.length === 0) {
      toast.error("Pregunta no puede estar vacía");
    } else {
      addNewQuestion({
        question,
        courseId: id,
  ...
  useEffect(() => {
    ...
    if (answerSuccess) {
      ...
      if (user.role !== "admin") {
        socketId.emit("notification", {
          ...
          message: `Tienes una nueva respuesta en ${data[activeVideo].title}`,
          userId: user._id,
        });
      }
    }
    ...
    if (reviewSuccess) {
      ...
      socketId.emit("notification", {
        ...
        message: `Tienes un nuevo comentario en ${course?.name}`,
        userId: user._id,
      });
    }
    if (reviewError) {
  ...
  const handleReviewReplySubmit = () => {
    if (!replyCreationLoading) {
      if (reply === "") {
        toast.error("Respuesta no puede estar vacía");
      } else {
  ...
  return (
    <div className="w-[95%] 800px:w-[86%] py-4 m-auto">
      ...
      <div className="w-full p-4 flex items-center justify-between bg-slate-500 bg-opacity-20 backdrop-blur shadow-[bg-slate-700] rounded shadow-inner">
        {["Descripción general", "Recursos", "Preguntas"].map((text, index) => (
          <h5
            key={index}
